...
try {
    // Locate standard system CA bundle across Linux distributions
    $ca_file = function_exists('curl_default_ca_info') ? (curl_default_ca_info()['default_cert_file'] ?? null) : null;
    if (!$ca_file || !file_exists($ca_file)) {
        $cert_locs = openssl_get_cert_locations();
        $ca_file = $cert_locs['default_cert_file'] ?? '/etc/ssl/certs/ca-certificates.crt';
    }
